...
Provides:
- 3D perspective/isometric views via pyrender, trimesh, and matplotlib fallback
...
class Renderer:
    ...
    def render_3d(self, shape: Any, view: ViewAngle = "iso",
        ...
        """Render 3D view with robust multi-backend fallback."""
        output_path = self.output_dir / filename

        # 1) pyrender
        try:
            path = self._render_3d_pyrender(shape, view, output_path)
            if not self._is_blank_image(path):
                return path
            print("pyrender produced blank image, trying trimesh")
        except Exception as e:
            print(f"pyrender failed ({e}), trying trimesh")

        # 2) trimesh image export
        try:
            path = self._render_3d_trimesh(shape, view, output_path)
            if not self._is_blank_image(path):
                return path
            print("trimesh produced blank image, trying matplotlib fallback")
        except Exception as e:
            print(f"trimesh failed ({e}), trying matplotlib fallback")

        # 3) deterministic fallback (always local, no GL context required)
    ...
    def _render_3d_trimesh(self, shape: Any, view: ViewAngle, output_path: Path) -> Path:
        ...
        # Apply deterministic camera transform by orienting scene camera target.
        # (trimesh may still depend on GL/pyglet for rasterization)
        try:
            ...
            raise RuntimeError(f"trimesh rendering failed: {e}") from e
    ...
    def _is_blank_image(self, path: Path) -> bool:
        """True if image exists but all pixels are effectively the same color."""
        if not path.exists():
            return True
        try:
            img = Image.open(path).convert("RGB")
            extrema = img.getextrema()  # [(min,max), (min,max), (min,max)]
            return all(ch_min == ch_max for ch_min, ch_max in extrema)
        except Exception:
            return True

    def _render_3d_matplotlib(self, shape: Any, view: ViewAngle, output_path: Path) -> Path:
        """Deterministic CPU fallback renderer using matplotlib mplot3d."""
        import matplotlib
        matplotlib.use("Agg")
        ...
        poly = Poly3DCollection(
            tris,
            facecolors=np.array(self.config.face_color) / 255.0,
            edgecolors=np.array(self.config.edge_color) / 255.0,
            linewidths=0.2,
            alpha=float(self.config.face_opacity),
        )
        ...
        elev, azim = elev_azim.get(view, (30, -45))
